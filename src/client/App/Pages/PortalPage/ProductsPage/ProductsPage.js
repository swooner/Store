...
const ProductPage = ( props ) =>  {
    ...
    const renderProducts = ( ) => {
        ...
        return product_list ? product_list.edges.map(( product, i ) => {
            // <Product> component is created below
            return (
                <Product key={ i } product={ product.node } deleteProduct={ deleteProduct } />
            )
        }) : []
    }
    ...
    return (
        <div className={ styles.ProductPage }>
            ...
            <Switch>
                <Route exact path='/portal/products/add-product' render={ () => {
                    return (
                        <AddProduct
                            { ...props }
                            category_list={ props } />
...
export default createFragmentContainer( ProductPage, {
    product_list: graphql`
        fragment ProductsPage_product_list on Query @argumentDefinitions (
            first: { type: "Int" }
        ) {
